...
class ConversionRegistry(ABC):
    _registered_conversions: dict[
        tuple[type, type], Callable[[Any], Result[Any, Any]]
    ] = {}
    ...
    def register_conversion(
        ...
        type_from: Type[A],
        type_to: Type[B],
        conversion: Callable[[A], Result[B, Any]],
    ) -> None:
    ...
    def _error_helper(cls, type_from: Type[A], type_to: Type[B]) -> str:
        ...
        if type_from in cls._f_map:
            msg.append(
                f"The only registered conversions from `{type_from.__name__}` are `{'`, `'.join(typ.__name__ for typ in cls.get_valid_targets(type_from))}`."
            )
        else:
            msg.append(
                f"There are no registered conversions from `{type_from.__name__}`."
            )
        if type_to in cls._r_map:
            msg.append(
                f"The only registered conversions to `{type_to.__name__}` are `{'`, `'.join(typ.__name__ for typ in cls.get_valid_sources(type_to))}`."
            )
        else:
            msg.append(
                f"There are no registered conversions to `{type_from.__name__}`."
            )
        return " ".join(msg)

    @classmethod
    def get_conversion(
        cls, type_from: Type[A], type_to: Type[B]
    ) -> Callable[[A], Result[B, Any]]:
        ...
        return cast(
            Callable[[A], Result[B, Any]],
            cls._registered_conversions[(type_from, type_to)],
        )
...
def register_conversion(
    type_from: Type[A], type_to: Type[B]
) -> Callable[[Callable[[A], Result[B, Any]]], Callable[[A], Result[B, Any]]]:
