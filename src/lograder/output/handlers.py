...
class HTMLHandler(logging.Handler):
    ...
    _TEMPLATE_PAGE: str = (_TEMPLATE_ROOT / "default_handler_page_template.html").read_text()
    _TEMPLATE_CARD: str = (_TEMPLATE_ROOT / "default_handler_card_template.htmf").read_text()

    def __init__(self, output_file: Optional[str] = None):
        super().__init__()
        self._items: list[HTMLItem] = []
        self._output_file: Optional[Path] = None if output_file is None else Path(output_file).resolve()
    ...
    @classmethod
    def _substitute_page(cls, **kwargs) -> str:
        return cls._substitute(cls._TEMPLATE_PAGE, **kwargs)

    @classmethod
    def _substitute_card(cls, **kwargs) -> str:
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        # can be raised lest there be an infinite exception loop.
            ...
            self.handleError(record)  # type: ignore[misc]
    ...
    def render_page_to_file(self) -> None:
        if self.output_file is None:
            raise DeveloperException("Tried to render HTML to file from `HTMLHandler`, but there is no assigned file.")
